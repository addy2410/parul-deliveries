...
const StudentRestaurants = () => {
  ...
  const filteredRestaurants = restaurantList.filter(restaurant => {
    const matchesSearch = restaurant.name.toLowerCase().includes(searchTerm.toLowerCase()) || restaurant.tags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase()));
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          duration: 0.5
            ...
            <div className="relative">
              <Input type="text" placeholder="Search for food or restaurant..." className="pl-10 py-6 pr-4 w-full rounded-lg border-gray-200 shadow-md" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        ...
        <ScrollArea>
          <div className="container mx-auto px-4 py-3 overflow-x-auto">
            <div className="flex space-x-2 min-w-max">
              {categories.map(category => <Button key={category} variant={activeCategory === category ? "default" : "outline"} className={activeCategory === category ? "bg-[#ea384c] hover:bg-[#d02e40]" : ""} onClick={() => setActiveCategory(category)}>
                  {category}
                </Button>)}
            </div>
          </div>
