...
const VendorOrdersList: React.FC<VendorOrdersListProps> = ({ 
      ...
      .on('postgres_changes', {
        ...
        if (payload.eventType === 'INSERT') {
          ...
          if (updated.status === 'delivered' || updated.status === 'cancelled') {
            ...
            // Notify parent component about delivery status update
          ...
          // Safely handle the deletion by checking if payload.old has the expected structure
  ...
  }, [vendorId, shopId, onOrderDelivered, onOrderUpdate]);
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      ...
      const { error } = await supabase
        ...
        .update({ 
          status: newStatus,
          // Add estimated delivery time if status is preparing
          ...(newStatus === 'preparing' ? { estimated_delivery_time: '30-45 minutes' } : {})
        })
        .eq('id', orderId);
      ...
      // Update local state for immediate UI feedback (the subscription will handle real-time updates)
      if (newStatus === 'delivered') {
        setOrders(prev => prev.filter(order => order.id !== orderId));
        
        // Notify parent component about the delivered order
  ...
  const deleteOrder = async (orderId: string) => {
    try {
      ...
      // Update local state (the subscription should handle this too)
      setOrders(prev => prev.filter(order => order.id !== orderId));
